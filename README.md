...
- Install all networkx dependency by running **pip3 install networkx\[default\]**  
- Go to src/simulation/simulation.py, under run\_simulation, we have link\_failure\_thread which defines our thread that will run link failures and recoveries. Change the probability based on your liking here.  
- Go back to the src directory which contains the main.py file. The main.py contains the following parameters:

  - Topology: Use the keywords from the following: \[dense, line, ring, sparse, star, tree\]
  - Size of the graph: numbers representing the hosts
  - Repetition: How many times the simulation should run

- So to run a ring graph with 10 hosts and repeat the simulation 3 times, we do: **sudo python3 main.py ring 10 3**
